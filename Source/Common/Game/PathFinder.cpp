...
{
    //Safety check that the state is idle, if it isn't a path is either already found,
	//or in the state of being found
...
void PathFinder::update(double aDelta)
{
	...
	while(isSearchingPath() == true && m_DestinationTileIndex != -1)
	{
		...
		if(currentNodeTileIndex == m_DestinationTileIndex)
		{
			...
			//to track back through the path nodes until all the way back to the start
		...
		//If we got here that means we haven't reached our destination path
		...
		for(int i = 0; i < adjacentTiles.size(); i++)
		{
			...
			{
				//If it does exist in the open list compare the scores..
				//keep the one with the lower F score, which reall mean..
				...
				//If the tile has a lower F score, update the G score and parent node
